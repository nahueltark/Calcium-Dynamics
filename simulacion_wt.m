...
for j=1:celulas
    ...
    % Asignación de alturas de ráfagas de acuerdo con las distribuciones generadas
    clear H L aux1 aux2 aux3 rH rL u s
    H=abs(random(pdH,nroH,1));
    L=abs(random(pdL,nroL,1));
    Y=abs(random(pdY,nroY,1));
    % Tiempos de ocurrencia de los eventos generados aleatoriamente
%     rH = random('Uniform',0,6000,nroH,1);
    ...
    if usarSigmoide
        ...
        rH = random('Uniform',0,6000,nroH,1);
        aux1(:,2) = [H; L; 0];% Altura de ráfaga sin modulación
    end
    aux1(:,3)=[rH; rL; 6000]; %tiempo de ráfaga
    aux1(:,1)=j;
        ...
        if (contadortau + tau) < wt(i-1,3)
            if modoYVC1 == 1
                ...
                if wt(i-1,2) > umbral_YVC1
                    dCay = dcaY_0 * (x0(2) - x0(1)) * (1 - exp((contadortau - wt(i-1,3)) / tau)) / (ca_0_vac - ca_0_cit);
                    contadortau = wt(i-1,3);
%                     dCay=0;
                    disp(['dCay ' num2str(dCay)])
                    x0(1) = x0(1) + dCay;
                    x0(2) = x0(2) - dCay * (((diametro_cell^3) * 0.5 + (((diametro_cell + 0.0005 * wt(i-1,3)) * (diametro_cell^2)) * 0.5) - diametro_vac^3) / diametro_vac^3);
                end
            ...
            elseif modoYVC1 == 2
                ...
                dCay = f * dcaY_0 * (x0(2) - x0(1)) * (1 - exp((contadortau - wt(i-1,3)) / tau)) / (ca_0_vac - ca_0_cit);
                contadortau = wt(i-1,3);
    ...
    [tsR b]=unique(ts);
    [pk_values, locs] =findpeaks((y(b,1)-ca_0_cit)/ca_0_cit,'MinPeakDistance',35,'MinPeakHeight',umbral_detec);
    pk_times = tsR(locs);% Asigna los tiempos correspondientes
